...
*/
  ...
  getById(id: any): Observable<any[]> {
    return this._httpClient.get<any[]>(environment.API_URL + '/api/quest-sub-type/' + id).pipe(
      tap((meterial) => {
        this._materials.next(meterial);
      })
    );
  ...
  update(data: any, id): Observable<any> {
    return this._httpClient.put(environment.API_URL + '/api/quest-sub-type/' + id, data, this.httpOptionsFormdata).pipe(
      switchMap((response: any) => {
        // Return a new observable with the response
        return of(response);
      })
  ...
  getPage(dataTablesParameters: any): Observable<DataTablesResponse> {
    return this._httpClient.get(environment.API_URL + '/api/quest-sub-type/page?page=1&limit=10', dataTablesParameters).pipe(
      switchMap((response: any) => {
        return of(response);
      })
    );
